...
那在`esm`模式下，即使可以`eval`，也毫无意义，因为它没有往全局挂载任何东西（除非你手动这么去做）
...
npm加载，如果是monorepo自然皆大欢喜，如果是在不同仓库，也可以模块联邦，这样开发时就没什么问题了。而在其他方面，这并不是`也行`的级别，实际上要好得多：更加优越的性能，更贴合组件化，甚至少了qiankun这一层，更好的debug/sourcemap...
...
1. 把`document`隔离了，所有dom自然就在web component内部，卸载时，把web component卸载就行，
...
## 微前端<<微服务
额，不提这个项目了，也暂时先将微前端的各种技术放一放，退回到这个概念的本身--微前端发起之初，是想做前端的微服务，对吧？
